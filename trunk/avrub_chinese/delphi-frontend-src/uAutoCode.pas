  ...
  try
    ...
    begin
      try
        ...
        Writeln(f, '                 http://sourceforge.net/projects/avrub');
        Writeln(f, '                 ');
        Writeln(f, '  Date:          2008.4');
        ...
        begin
          Writeln(f, '//加密算法');
          Writeln(f, '// 0: PC1_128, 128位 PC1 加密算法');
          Writeln(f, '// 1: PC1_256, 256位 PC1 加密算法');
        end
        else
        begin
          Writeln(f, '//crypt algorithm');
          Writeln(f, '// 0: PC1_128, PC1 Cipher Algorithm with 128 bits key');
          Writeln(f, '// 1: PC1_256, PC1 Cipher Algorithm with 256 bits key');
        end;
        Writeln(f, '');
        Writeln(f, '#define Algorithm          ',cbbAlgorithm.ItemIndex);
        Writeln(f, '');
        Writeln(f, '#if Decrypt');
        Writeln(f, '');
        Writeln(f, '#if (Algorithm == 0)');
        if b then
        ...
        for i := 0 to 3 do
        begin
          Write(f,'  0x',IntToHex(buf[i*4],2),', 0x',IntToHex(buf[i*4+1],2),', 0x',IntToHex(buf[i*4+2],2),', 0x',IntToHex(buf[i*4+3],2));
          if i = 3 then
            Writeln(f)
          else
            Writeln(f,',');
